...
- [ ] 6.5a Implement environment-controlled read-only `git`: neutralise system, global and repository-directory configuration, force a non-interactive pager, disable terminal prompting, and permit only allowlisted read-only subcommands and flags. Verify the two live exploits are closed — `git -c alias.pwn='!id' pwn` and `git --git-dir=<in-root bare repo> pwn`, **both confirmed executing arbitrary shell on 2026-08-28** — and that an alias planted in a reachable repository does not execute
- [ ] 6.5b Verify the shipped default command set passes the admission test: for each command, state the bounded grammar under which it cannot name another program or script and cannot reach an unchecked path. Any command failing that test must be absent from the default set
...
- [ ] 7.1 Verify no delete-effect command is present in the shipped default policy, and that `rm` remains unavailable exactly as in 1.x — zero regression; verify a test asserts a delete request is refused with a message naming the deferral
