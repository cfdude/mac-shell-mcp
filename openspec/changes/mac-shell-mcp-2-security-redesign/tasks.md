...
- [ ] 15.1a Enable GitHub private vulnerability reporting on the repository — `security_and_analysis` does not report it as enabled, yet `SECURITY.md` now links to it as the preferred route; verify the advisory-submission form is reachable by an unauthenticated third party
