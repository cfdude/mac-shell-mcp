...
- [ ] 13.2b File an npm dispute/DMCA over `mac-shell-mcp@1.0.4` on the grounds that it redistributes this repo's MIT-licensed code with attribution removed and no license text, and separately request npm deprecate it as vulnerable; verify by recording the npm support ticket reference
...
- [ ] 15.1 Replace the vulnerability reporting contact at `SECURITY.md:15`, currently the unconfigured template placeholder `[security@your-email.com]` — the documented channel for four advisories that went untriaged, the oldest for 211 days. Point it at GitHub private vulnerability reporting, and verify a test report reaches a monitored destination
...
- [ ] 15.3 Fix the false security claims that three reporters explicitly cited: `README.md:255` and `SECURITY.md:28` both state the server "uses `execFile` instead of `exec` to prevent shell injection", and `smithery.json` repeats the "secure command whitelisting" claim; verify no file asserts a guarantee the code does not provide
...
- [ ] 15.6 Enable branch protection on `main` (currently none: no required review, no required checks, force-push unrestricted) and enable Dependabot security updates (currently disabled, with no `.github/dependabot.yml`); verify by re-querying the branch protection and vulnerability-alerts endpoints
