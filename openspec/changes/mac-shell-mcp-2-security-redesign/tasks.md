...
- [ ] 3.5a Implement winning-source change detection: record on each run both which source supplied policy and its filesystem identity, report at startup when either differs from the previous run, and do not adopt the new source unattended. The previous-run record is itself a protected location; verify a test asserts a same-path file replacement is detected, not just a change of discovery slot
...
- [ ] 6.5b Verify the shipped default set is exactly `ls`, `pwd`, `echo`, `cat`, `head`, `tail`, `wc`, `grep` (NOT `rg` — it installs outside the system program directories, and admitting `/opt/homebrew/bin` would make a user-writable directory a program source, verified `drwxrwxr-x` on this machine), and that each carries a stated flag allowlist under which it cannot execute another program, **cannot write or create any path**, and cannot read configuration from its environment; verify the enumeration in the spec matches the shipped policy exactly
