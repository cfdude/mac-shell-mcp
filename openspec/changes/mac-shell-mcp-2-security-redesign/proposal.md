...
- **BREAKING** — tools split by confinement so a host's never/always/ask setting is meaningful: `execute_command` (inside roots) and `execute_external_command` (outside roots), plus `execute_pipeline`, `get_policy`, `suggest_policy_config`. All carry MCP tool annotations, currently unset.
